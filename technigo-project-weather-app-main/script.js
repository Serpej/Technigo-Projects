...
const fetchDataTodaysForecast = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const response = yield fetch("https://api.openweathermap.org/data/2.5/weather?q=Stockholm,Sweden&units=metric&APPID=798eeac13b738f9ebfbf936c1e8564a9");
        if (!response.ok) {
        ...
        const sunriseTimeTwoDigits = sunriseTime.toLocaleTimeString("en-GB", {
            hour: "2-digit",
            minute: "2-digit"
        });
        const sunsetTimeTwoDigits = sunsetTime.toLocaleTimeString("en-GB", {
            hour: "2-digit",
            minute: "2-digit"
        });
...
const fetchDataNextFiveDaysForecast = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const response = yield fetch("https://api.openweathermap.org/data/2.5/forecast?q=Stockholm,Sweden&units=metric&APPID=798eeac13b738f9ebfbf936c1e8564a9");
        if (!response.ok) {
            throw new Error(`HTTP Error! Status: ${response.status}`);
        }
        const data = yield response.json();
        ...
        eachDay.forEach((day) => {
            ...
            if (time.includes("12:00:00")) {
                const dateOfTime = new Date(time);
                const weekDay = dateOfTime.getDay();
